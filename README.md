...
- [Awesome Jev Robustness](https://github.com/Yifan-Lan/awesome-jev-robustness) — Curated index of independent Jev tests grouped by what they measured (calibration, consistency, wording and option perturbation, prompt injection, abstention, failure modes, cross-language), each with model version and sample size; numbers are as reported by the original authors, and task benchmarks without a robustness angle are kept in a separate data file.
