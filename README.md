...
- [Luce](https://github.com/scienthoon/luce) - Open recipe for Jev-style decision models: a task description → an LLM teacher writes the data → LoRA + decision head on Qwen3-4B-Base → calibrated choice/score/boolean probabilities on a 12 GB GPU; README reports accuracy and ECE against Jev on identical test items, including where training does not help. Replay demo: https://huggingface.co/spaces/noscienthoon/luce-live-triage
