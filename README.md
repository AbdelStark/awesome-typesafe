...
- [Jev Plays Pokémon](https://github.com/anxkhn/JevPlaysPokemon) — FireRed ROM and Showdown harness that turns privileged battle state into a typed Choice so Jev can pick a legal move or switch, with a live dashboard and recorded level-40 Elite Four wins; each match restores the configured party, so it is not a no-heal campaign.
