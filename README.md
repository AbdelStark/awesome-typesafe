...
- [pi-heed](https://github.com/Nyarlathoteppppp/pi-heed) — Pi extension that extracts constraints from the user's own messages (English and Chinese) and checks side-effecting tool calls against them before they run, using deterministic rules for side-effect detection and Jev for free-text constraints, paraphrased constraints, and a second opinion on exceptions the user granted later; experimental, shadow mode by default, fails open, and its thresholds are checked only on small hand-written cases.
