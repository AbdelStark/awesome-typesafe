...
- [oxlint-plugin-jev](https://github.com/wobsoriano/oxlint-plugin-jev) — Oxlint plugin with one rule, `jev/ask`, that turns plain-English yes/no questions about functions, calls, JSX elements, or files into Jev checks and reports an error above a cutoff you choose; experimental, sends matched snippets to TypeSafe, and warns rather than fails when Jev is unreachable unless `ci` is set to `"fail"`.
